#include "iecdosinterpreter.h"

// Bytes of the compiled Flapper sample game from C64Studio
static byte prg[] = {  };

IECDOSInterpreter::IECDOSInterpreter() {
  state = DOS_STATE_ACCEPTING_COMMAND;

  transReceiver = new IECTransreceiver(this);
  transReceiver->reset();
  transReceiver->start();
}

IECDOSInterpreter::~IECDOSInterpreter() {
  delete transReceiver;
}

void IECDOSInterpreter::onBecameTalker() {
  // TODO: do something more complex check here about what should we do here :D
  state = DOS_STATE_BEGIN_OF_FILE_SENDING;
}

void IECDOSInterpreter::onChannelOpenCommand(byte channel) {
  // TODO Write code
}

void IECDOSInterpreter::onChannelCloseCommand(byte channel) {
  // TODO Write code
}

void IECDOSInterpreter::sendNextFileByte() {
  // TODO: this loads sample game from firmware. Replace it with something more usable
  if (transReceiver->isTalker()) {

    if (transReceiver->isTransmitting()) {
      return;
    }

    transReceiver->sendByte(prg[currentSendingFileByte], currentSendingFileByte == sizeof(prg) - 1);
    currentSendingFileByte++;
    if (currentSendingFileByte == sizeof(prg)) {
      state = DOS_STATE_ACCEPTING_COMMAND;
    }

  } else {
    // Something bad happened
    Serial.print("!");
    state = DOS_STATE_ACCEPTING_COMMAND;
  }

}

void IECDOSInterpreter::doProcessingOnMainLoop() {
  switch (state) {
  case DOS_STATE_BEGIN_OF_FILE_SENDING:
      currentSendingFileByte = 0;
      state = DOS_STATE_SENDING_FILE;
      sendNextFileByte();
      break;
  case DOS_STATE_SENDING_FILE:
      sendNextFileByte();
      break;
  default:
    break;
  }
}

void IECDOSInterpreter::onTransreceiverReset() {
  state = DOS_STATE_ACCEPTING_COMMAND;
}
